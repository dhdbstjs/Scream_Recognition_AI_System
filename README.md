...
**1. RDS로 데이터베이스 생성**
>1. MySQL로 선택하여 데이터베이스를 생성한다.
>
>![DB01](image/DB/DB01.png)
>
>![DB02](image/DB/DB02.png)
>
>2. 할당량을 초과할 때 자동으로 스토리지를 늘릴 경우 비용이 청구될 수 있으므로, 자동 조정은 반드시 꺼놓는다.
>
>![DB03](image/DB/DB03.png)

<br/>

**2. RDS와 HeidiSQL 연동**
>1.	호스트명 / IP는 RDS로 생성한 데이터베이스의 엔드 포인트 값으로 한다.
>2.	사용자와 암호, 포트는 데이터베이스를 생성할 때 입력한 값으로 한다.
>
>![DB04](image/DB/DB04.png)

<br/>

**3. 파라미터 그룹 생성**
>1. MySQL 최신 버전으로 선택하고, 관리가 편하도록 생성한 DB와 연관된 이름으로 지정한다.
>
>![DB05](image/DB/DB05.png)
>![DB06](image/DB/DB06.png)

<br/>

**4. RDS 파라미터 그룹 수정**
>1. 기본 언어를 한글로 설정하기 위해 생성한 파라미터의 옵션을 수정한다. <br/>
‘char’ 검색을 통해 나오는 모든 파라미터의 값을 ‘utf8’로 변경한다.
>   
>![DB07](image/DB/DB07.png)
>
>2. ‘collation’ 검색을 통해 나오는 모든 파라미터의 값을 ‘utf8-general_ci’로 변경한다.
>
>![DB08](image/DB/DB08.png)
>
>3. 데이터베이스 수정을 통해 DB 옵션 그룹을 생성한 파라미터 그룹의 DB 패밀리로 수정한다.
>
>![DB09](image/DB/DB09.png)

<br/>

**5. 인바운드 규칙 생성하기**
>1. RDS 데이터베이스 정보에서 VPC 보안 그룹에 들어간다.
>
>![DB10](image/DB/DB10.png)
>
>![DB11](image/DB/DB11.png)
>
>2. 유형은 MYSQL/Aurora로 선택하고 소스는 사용자 지정으로 하여 IP 주소를 지정한다. <br/>
지정한 IP 주소에서만 DB 접근이 가능하므로 본인이 사용할 장소의 IP 장소로 설정한다.
>
>![DB12](image/DB/DB12.png)
>
>3. 새 인바운드 규칙을 생성하여 여러 장소를 지정할 수도 있다. 규칙이 여러 개일 경우 각 구분을 하기 위해 설명을 통해 이름을 지어주는 것이 좋다. <br/>
*‘0.0.0.0/0’으로 할 경우 장소에 상관없이 DB 접근이 가능하지만 보안상 좋지 않은 방법이다.
>
>![DB13](image/DB/DB13.png)

<br/>

**6. DB와 웹 연동하기**
>1. HeidiSQL에 사용할 데이터베이스에 쿼리문을 통해 DB 테이블명과 컬럼을 생성한다.
>
>![DB14](image/DB/DB14.png)
>
>2. KEY 값을 비롯한 비번 등의 정보는 타인에게 유출되면 안 되기 때문에 코드에 직접 적는 것보다 json 파일을 생성해 입력 후 호출하여 사용하는 것이 좋다.<br/>
VSCode에서 HeidiSQL 프로그램에 로그인할 때의 입력값과 사용하는 데이터베이스 명을 입력한다.
>
>![DB15](image/DB/DB15.png)
>
>3. DB 정보 json 파일을 불러와 코드에 적용한다.
>
>![DB16](image/DB/DB16.png)
>
>4. DB 테이블을 보여주는 API를 제작한다. 사용자가 지정한 경로 서버 (5000/api/ saferoad)에 접속하면 데이터베이스 쿼리의 테이블의 값을 가져온다.
rows를 변수로 가져와 모든 값을 사용자에게 제공한다.
>
>![DB17](image/DB/DB17.png)
>
>5. 클라이언트에서 API를 호출하고 fetch 함수를 통해 서버에서 지정한 경로의 DB 정보를 가져온다. 해당 코드를 통해 DB 데이터를 json 형태의 데이터로 확인할 수 있다.
>
>![DB18](image/DB/DB18.png)

<br/>

**7. S3 버킷과 웹 연동하기**
>1. AWS SDK를 사용하여 Node.js에서 AWS S3 버킷 파일을 가져온다. <br/>
VSCode 프로그램 터미널에 ‘npm install aws-sdk'를 입력해 설치한다.
>2. AWS 액세스 키와 비밀키는 외부 유출을 막기 위해 json 파일에 따로 입력한다. AWS 액세스 키 정보를 불러오고 AWS 리전을 입력한다.
>
>![DB19](image/DB/DB19.png)
>
>3. S3 버킷에서 가져올 파일명과 해당 버킷의 이름을 지정한다.
>
>![DB20](image/DB/DB20.png)
>
>4. 서버에서 AWS에 접근할 경우 CORS 보안 정책에 의해 접근이 허용되지 않을 수 있다. Amazon S3 버킷 권한에 있는 CORS에  코드를 추가해 준다.
>
>![DB21](image/DB/DB21.png)
