...
**4. RDS 파라미터 그룹 수정**
>1. 기본 언어를 한글로 설정하기 위해 생성한 파라미터의 옵션을 수정한다. ‘char’ 검색을 통해 나오는 모든 파라미터의 값을 ‘utf8’로 변경한다.
>   
...
><br/><br/><br/>
>2. 유형은 MYSQL/Aurora로 선택하고 소스는 사용자 지정으로 하여 IP 주소를 지정한다. 지정한 IP 주소에서만 DB 접근이 가능하므로 본인이 사용할 장소의 IP 장소로 설정한다.
>
>![DB12](image/DB/DB12.png)
><br/><br/><br/>
>3. 새 인바운드 규칙을 생성하여 여러 장소를 지정할 수도 있다. 규칙이 여러 개일 경우 각 구분을 하기 위해 설명을 통해 이름을 지어주는 것이 좋다.
...
><br/><br/><br/>
>5. 클라이언트에서 API를 호출한다. fetch 함수를 통해 서버에서 지정한 경로의 DB 정보를 가져온다. 해당 코드를 통해 DB 데이터를 json 형태의 데이터로 확인할 수 있다.
>
...
**7. S3 버킷과 웹 연동하기**
>1. AWS SDK를 사용하여 Node.js에서 AWS S3 버킷 파일을 가져온다. VSCode 프로그램 터미널에 ‘npm install aws-sdk'를 입력해 설치한다.
